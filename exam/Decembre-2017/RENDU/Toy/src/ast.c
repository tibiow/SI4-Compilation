 ...
 * Last file update: 18-Dec-2017 19:24 (eg)
 */
...
static void free_identifier(ast_node *node) {
  ...
  if (IDENT_PREFIX(node)) free(IDENT_PREFIX(node));
  // Obviously, do not free the in_class slot
  // Do not free member which is an element of the class table and will serve
  // for next calls.
  free(node);
}
...
ast_node *make_identifier(char *id) {
  ...
  p->value     = id;
  p->in_class  = NULL;  // will  be changed later if we have a field
  p->prefix    = NULL;  // will be changed later if we have a field
  p->qualified = NULL;  // will be changed after analysis.
  return (ast_node *) p;
}

ast_node *add_prefix_to_identifier(ast_node *ident, ast_node *prefix) {
  IDENT_PREFIX(ident) = prefix;
  return ident;
}
...
ast_node *int_type, *float_type, *bool_type, *string_type, *void_type, *null_type;
...
//  ---- Class declaration ----------------------------------------
static void free_class(ast_node *node) {
  // Nothing to do. A class life extent is the program one
}


ast_node *make_class(ast_node *name, ast_node *parent, List members) {
  DEF_AST(p, class, make_type(IDENT_VAL(name), "NULL"));
  p->name    = name;
  p->members = members;
  p->parent  = parent;
...
//  ---- Instance creation ----------------------------------------
static void free_instance_new(ast_node *node) {
  free(INSTANCE_NEW_NAME(node));
  free(node);
}

ast_node *make_instance_new(ast_node *name) {
  DEF_AST(p, instance_new, make_type(IDENT_VAL(name), "NULL"));
...
// ---- This designator -------------------------------------------
static void free_this(ast_node *name) { }
...
// ---- Super designator -------------------------------------------
static void free_super(ast_node *name) { }

ast_node *make_super(void) {
  DEF_AST(p, super, NULL);
  return (ast_node*) p;
}
...
/* ---- READ ------------------------------------------------------------ */
static void free_read_statement(ast_node *node){
  list_destroy(((struct s_print_statement *)node)->parameters);
  free(node);
}
...
/* ---- BREAK ------------------------------------------------------------ */
static void free_break_statement(ast_node *node){ free(node); }
...
static void free_try_statement(ast_node *node){
  struct s_try_statement * s =  (struct s_try_statement *) node;
  free(s->try_stmt); free(s->catch_stmt); free(s->finally_stmt);
  free(node);
}
...
ast_node *make_try_statement(ast_node *try, ast_node *catch, ast_node *finally) {
  ...
  p->try_stmt     = try;
  p->catch_stmt   = catch;
  p->finally_stmt =  finally;
  return (ast_node *) p;
}

/* ---- THROW ------------------------------------------------------------ */
static void free_throw_statement(ast_node *node){ free(node); }
...
void init_ast(void)
{
  ...
  null_type   = make_standard_type("void *",      "NULL");
}
