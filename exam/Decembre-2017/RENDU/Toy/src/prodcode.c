 ...
 * Last file update: 18-Dec-2017 19:18 (eg)
 */
...
//
// ======================================================================
static void produce_function_body(struct s_function *fct) {
  ast_node *body = fct->body;
  code(body);
}
...
void produce_code_function(ast_node *node) {
  ...
  emit("%s ", TYPE_NAME(AST_TYPE(node))); emit_function_name(node);
  ...
  // Body or ';'
  if (n->body) { produce_function_body(n); } else emit (";\n");
}
      ...
      emit("(%s) this", klass_id);
        ...
        if(debug_mode){
          if(AST_TYPE(method)==void_type){
            ...
            emit("ENTER_FUNC(");code(AST_TYPE(method)); emit(")");
          }
        }
        else{
          emit("return");
