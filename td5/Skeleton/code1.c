...
float eval(ast_node *n) {
  ...
  switch (AST_KIND(n)) {
    case k_number:    return NUMBER_VALUE(n);
    case k_ident:     return get_ident_value(VAR_NAME(n));
    case k_operator: {
      ...
      switch (OPER_OPERATOR(n)) {
        ...
        case KIF:   if (eval(op[0])){eval(op[1]); } else{eval(op[2]);} return 0;
        case KWHILE:  while(eval(op[0])){eval(op[1]);} return 0;
        case '=': return set_ident_value(VAR_NAME(op[0]), eval(op[1]));
        case KREAD: printf("%s? ",VAR_NAME(op[0])); int num; scanf("%d",&num);  return set_ident_value(VAR_NAME(op[0]),num);
        default:        error_msg("Houston, we have a problem: unattended token %d",OPER_OPERATOR(n));
        return 0;
      }
    }
    default:
