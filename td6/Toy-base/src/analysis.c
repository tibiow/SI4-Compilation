 ...
 * Last file update: 18-Nov-2015 21:41 (eg)
 */
    ...
    case 2: analysis(op1); analysis(op2);
            ...
            switch(EXPRESSION_KIND(node)) {
              ...
              case barith:
                ...
                if (AST_TYPE(op1) == int_type && AST_TYPE(op2) == int_type){
                  AST_TYPE(node) = int_type;
                }
                else if(AST_TYPE(op1) == string_type && AST_TYPE(op2) == string_type){
                  AST_TYPE(node) = string_type;
                }
                else if(AST_TYPE(op1) == float_type || AST_TYPE(op2) == float_type){
                  AST_TYPE(node) = float_type;
                }
                else{
                  error_msg(node, "operands of %s must be int", name);
                }
                break;
...
void analysis_switch_statement(ast_node *node) {
  ...
  FORLIST(p, n->listcond) {
    ...
    if(cpt % 2 == 0 && AST_TYPE(list_item_data(p)) != bool_type) {
      error_msg(node, "Cond in case must be boolean");
    }
    cpt++;
  
  }
  analysis(n->defaultcase);
}
