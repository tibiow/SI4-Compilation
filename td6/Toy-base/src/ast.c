 ...
 *
 * Copyright © 2014-2016 Erick Gallesio - I3S-CNRS/Polytech'Nice <eg@unice.fr>
 *
 *
 ...
 * Last file update: 22-Nov-2016 11:35 (eg)
 */
...
static void free_constant(ast_node *node) {
  if (AST_TYPE(node) == string_type) free(CONSTANT_STRING_VALUE(node));
  free(node);
}
...
ast_node *int_type, *bool_type, *string_type,*float_type, *void_type;
...
static void free_switch_statement(ast_node *node) {
  ...
  list_destroy(n->listcond);
  free(n->defaultcase);
  free(node);
}

ast_node *make_switch_statement(List listcond, ast_node *defaultcase) 
{
  DEF_AST(p, switch_statement, NULL);
  p->listcond = listcond;
  p->defaultcase = defaultcase;
  return (ast_node *) p;
}
