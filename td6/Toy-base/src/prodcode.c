...
void produce_code_expression(ast_node *node) {
  ...
  switch (EXPRESSION_ARITY(node)) {
    case 1:
    if(EXPRESSION_KIND(node)==pre){
      emit(name);
      code(EXPRESSION_OP1(node));
    }
    else if (EXPRESSION_KIND(node)==post)
    {
      code(EXPRESSION_OP1(node)); 
      emit(name);
    }
      ...
      switch(name[0]) {
        case '-':
          if((EXPRESSION_KIND(EXPRESSION_OP1(node))==post) || (EXPRESSION_KIND(EXPRESSION_OP1(node))==pre)){
            emit("%s ", name); code(EXPRESSION_OP1(node));
          }
          else{
  ...
  case 2:
    if (EXPRESSION_KIND(node)==comp && AST_TYPE(EXPRESSION_OP1(node))==string_type) {
      ...
      emit(", ");  code(EXPRESSION_OP2(node));
      emit(")");
    } else if ((strcmp(name, "+") == 0) && AST_TYPE(EXPRESSION_OP1(node))==string_type && AST_TYPE(EXPRESSION_OP2(node))==string_type){
      emit("_my_strcat("); code(EXPRESSION_OP1(node));
      emit(", ");      code(EXPRESSION_OP2(node));
      emit(")");
    } else {
...
void produce_code_print_statement(ast_node *node) {
  ...
  FORLIST(p, n->parameters) {
    ...
    }else if (AST_TYPE(item) == float_type) {                   // bool
      emit("printf(\"%%f\", ");
...
void produce_code_switch_statement(ast_node *node) {
  ...
  // produce for2 (the condition of the while
  indent(0); emit("if("); if (n->for2) code(n->for2); else emit("1"); emit("){\n");
...
void produce_code_switch_statement(ast_node *node) 
{
  ...
  if(list_size(n->listcond) == 0 && n->defaultcase == NULL ) {
    return;
  }
  else if(list_size(n->listcond) == 0 && n->defaultcase != NULL) {
    emit("{");
    code(n->defaultcase);
    emit("}");
  }
    ...
    FORLIST(p, n->listcond) {
      if(cpt%2 == 0) {
        ...
        emit("{");
        indent(0);code(list_item_data(p));
        emit("}");
      }
      cpt++;
    }
