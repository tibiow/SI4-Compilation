 ...
 * Last file update: 16-Nov-2015 15:38 (eg)
 */
...
%token KWHILE KIF KPRINT KRETURN KBREAK KFOR KSWITCH KDEFAULT KCASE
%token TINT TBOOL TSTRING TVOID TFLOAT
...
stmt:		';'									{ $$ = make_expr_statement(NULL); }
    ...
    | 	KIF '(' expr ')' stmt %prec IFX         { $$ = make_if_statement($3, $5, NULL); }
    | 	KIF '(' expr ')' stmt KELSE stmt 		{ $$ = make_if_statement($3, $5, $7); }
	|	KFOR '(' for1 ';' for2 ';' for3 ')' stmt { $$ = make_for_statement($3, $5, $7, $9); } 
	|   KSWITCH '{' cond_list defcond '}' 		{ $$ = make_switch_statement($3, $4);}
	|	var_decl ';'							{ $$ = $1;}
	|	prototype ';'							{ $$ = $1;}
    |	error ';'								{ yyerrok; $$ = NULL; }
	;
...
cond_list:      cond_list KCASE expr ':' stmt { list_append($1, $3, FREE_NODE); list_append($1, $5, FREE_NODE);$$ = $1; }
            |       /* empty */             { $$ = list_create(); }
            ;

defcond :       KDEFAULT ':' stmt       { $$=$3; }
            |       /* empty */			{  $$ = NULL; }
            ;
// ========== FOR ==========
for1:           var_decl                { $$ = $1; }
...
expr:
	...
	|	PLUSPLUS var 		{ $$ = make_expression("++",  pre, 1, $2);}
	|	MOINSMOINS var 		{ $$ = make_expression("--",  pre, 1, $2);}
	|	var PLUSPLUS 		{ $$ = make_expression("++",  post, 1, $1);}
	|	var MOINSMOINS 		{ $$ = make_expression("--",  post, 1, $1);}
	|	expr POWER expr 	{ $$ = make_expression("**",  barith, 2, $1, $3);}
	;
