 ...
 * Last file update:  6-Dec-2015 18:04 (eg)
 */
...
static bool is_object(ast_node *n){
  return AST_TYPE(n) ==  Object_class;
}
...
static void simple_identifier_analysis(ast_node *node, char *id){
  ...
  if (!var) {
    ...
    error_msg(node, "%s class used as a variable", id);
  }
    ...
    case 2: analysis(op1); analysis(op2);
            ...
            switch(EXPRESSION_KIND(node)) {
              case assign:
                ...
                break;
              case barith:
              if (!is_object(op1) || !is_object(op2)){
                error_msg(node, "operands of %s must be objects", name);
              }
              else {
