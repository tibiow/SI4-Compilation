 ...
 * Last file update:  6-Dec-2015 17:51 (eg)
 */
...
%token <null_value>  TNULL
%token <string_value> STRING
%token <null_value>
%token <ident> IDENTIFIER
%token KWHILE KIF KPRINT KRETURN KBREAK KFOR KREAD KINSTANCEOF
%token TINT TFLOAT TBOOL TSTRING TVOID TTNULL
%token KCLASS KNEW KEXTENDS KTHIS 

%type   <ast>   declarations declaration
%type   <ast>   stmt expr var_decl type var identifier call prefix designator
%type   <ast>   class_decl attr_decl inherit
%type   <ast>   func_decl prototype expr_opt init_var
%type   <ast>   for1 for2 for3
%type   <lst>   fparam_list eparam_list stmt_list param_read
...
stmt:           ';'                     { $$ = make_expr_statement(NULL); }
        ...
        |       KREAD '(' param_read ')' ';'  { $$ = make_read_statement($3); }
...
param_read:     param_read ',' var        { list_append($1, $3, FREE_NODE); $$ = $1; }
        |       var                       { $$ = list_create();list_append($$, $1, FREE_NODE); }
        |        /* empty */             { $$ = list_create(); }
        ;
...
// ========== CLASSES ==========
class_decl:     KCLASS var ';'          { $$ = make_class($2, NULL, NULL); }
        |       KCLASS var inherit '{' members_list '}' opt_semi
                                        { $$ = make_class($2, $3, $5); }
        ;
...
expr:
        ...
        |       expr KINSTANCEOF expr { $$ = make_expression("instanceof", comp, 2, $1,$3);}       
